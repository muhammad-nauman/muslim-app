        ...
        <div class="col-md-12">
            ...
            <form class="form-horizontal" enctype="multipart/form-data" method="POST" action="{{ route('weekly_reminders.update', ['weekly_reminder' => $weeklyReminder->id]) }}">
                    ...
                    <div class="col-lg-7">
                        <select class="form-control m-b required" name="category_id">
                            ...
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" @if( $weeklyReminder->category_id == $category->id) selected @endif >{{ $category->name }}</option>
                            @endforeach
                        </select>
                        @error('category_id')
                ...
                <div class="form-group @error('publishing_timestamp') has-error @enderror"><label class="col-lg-2 control-label">Publish Date & Time</label>
                    <div class="col-lg-7">
                        <input readonly type="text" name="publishing_timestamp" placeholder="Publish Date & Time" id="publishing_timestamp" class="form-control" value="{{ $weeklyReminder->publishing_timestamp }}">
                ...
                <div class="form-group @error('content') has-error @enderror pace-inactive article"><label class="col-lg-2 control-label">Article Content</label>
                    <div class="col-lg-7">
                        <textarea id="summernote" name="content">{{ $weeklyReminder->content }}</textarea>
                    </div>
                </div>
